...
const INITIAL_STATE = [
  {
    ...
    assignees: [{ id: 'user-1', name: 'Rafiq', role: 'PM', link: '', notes: '', userId: '' }],
  ...
  },
  {
    ...
    assignees: [{ id: 'user-2', name: 'Zafar', role: 'UXer', link: '', notes: '', userId: '' }],
  ...
  },
  {
    ...
    assignees: [{ id: 'user-3', name: 'Anis', role: 'Designer', link: '', notes: '', userId: '' }],
  ...
  },
  {
    ...
    color: '#85E3B2',
    todos: [],
    assignees: [{ id: 'user-4', name: 'Mitu', role: 'Developer', link: '', notes: '', userId: '' }],
  ...
  },
  {
    ...
    color: '#D3ADF7',
    todos: [],
    assignees: [{ id: 'user-5', name: 'Core', role: 'Adviser', link: '', notes: '', userId: '' }],
    notes: [{ id: 105, text: 'Research Market', assigneeId: 'user-5' }, null, null]
  }
];

// ── Calendar Availability Panel ──────────────────────────────────────────────
function CalendarAvailability({ userId }) {
  ...
  // Calculate how many working hours are blocked
  const totalBusyHours = (data?.busy || []).reduce((acc, slot) => {
    const hrs = (new Date(slot.end) - new Date(slot.start)) / 3_600_000;
    return acc + hrs;
  }, 0);

  const workingHoursInSprint = 14 * 8; // 14 days × 8 hrs
  const availableHours = Math.max(0, workingHoursInSprint - totalBusyHours);
  ...
  const formatSlot = (slot) => {
    ...
    return sameDay
      ? `${s.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' })}  ${s.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })} – ${e.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}`
      : `${fmt(s)} → ${fmt(e)}`;
  };

  if (loading) {
    return (
      <div className="text-[10px] text-[var(--text)] opacity-60 py-2 animate-pulse">
        Loading calendar…
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-[10px] text-red-400 py-1">
        {error.includes("hasn't connected")
          ? "This member hasn't connected Google Calendar yet."
          : error}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-[var(--text)] mb-1">
        <span>Sprint Capacity (next 14 days)</span>
        <span className={availableHours < 40 ? 'text-red-400' : 'text-green-500'}>
          {Math.round(availableHours)}h free
        </span>
      </div>
      <div className="w-full h-2 bg-[var(--color-muted)] rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${availableHours < 40 ? 'bg-red-400' : 'bg-green-500'}`}
          style={{ width: `${Math.min(100, (availableHours / workingHoursInSprint) * 100)}%` }}
        />
      </div>
      <p className="text-[9px] text-[var(--text)] opacity-60">
        {Math.round(totalBusyHours)}h blocked of {workingHoursInSprint}h total
      </p>

      {/* Busy Slots List */}
      {data.busy.length === 0 ? (
        <p className="text-[10px] text-green-500 mt-1">✓ No conflicts — fully available this sprint</p>
      ) : (
        <div className="flex flex-col gap-1 mt-1 max-h-36 overflow-y-auto pr-1">
          <p className="text-[10px] font-bold text-[var(--text)] uppercase tracking-wider">
            Busy Blocks ({data.busy.length})
          </p>
          {data.busy.map((slot, i) => (
            <div
              key={i}
              className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 rounded px-2 py-1 font-mono"
            >
              🔴 {formatSlot(slot)}
            </div>
          ))}
      ...
      <button
        onClick={load}
        className="text-[9px] text-[var(--text)] opacity-50 hover:opacity-100 mt-1 cursor-pointer text-left"
      >
        ↻ Refresh
      </button>
    </div>
...
export default function SprintBoard() {
  ...
  const [noteModal, setNoteModal] = useState({
    isOpen: false, text: '', deptId: null, index: null, noteId: null, assigneeId: ''
  });

  const [deptModal, setDeptModal] = useState({
    isOpen: false, mode: 'add', deptId: null, name: '', assets: ''
  });

  const [assigneeModal, setAssigneeModal] = useState({
    isOpen: false, deptId: null, name: '', role: ''
  });

  const openAddDepartmentModal = () => {
    setDeptModal({ isOpen: true, mode: 'add', deptId: null, name: '', assets: 'Custom managed track assets and parameters...' });
  };

  const openEditDepartmentModal = (deptId, currentName, currentAssets) => {
    setDeptModal({ isOpen: true, mode: 'edit', deptId, name: currentName, assets: currentAssets || '' });
  };
  ...
  const handleSaveDepartment = () => {
    ...
    if (deptModal.mode === 'add') {
      setDepartments(prev => [...prev, {
        ...
        todos: [], assignees: [], notes: [null, null, null]
      }]);
    } else {
      setDepartments(prev => prev.map(d =>
        d.id === deptModal.deptId ? { ...d, name: deptModal.name.trim(), assets: deptModal.assets.trim() } : d
      ));
    }
    ...
    setDepartments(prev => prev.map(d => {
      ...
      return {
        ...
        assignees: [...d.assignees, {
          id: `user-${Date.now()}`,
          name: assigneeModal.name.trim(),
          role: assigneeModal.role.trim() || 'Member',
          link: '', notes: '', userId: ''
        }]
      };
    }));
  ...
  const handleUpdateMemberField = (deptId, memberId, field, value) => {
    setDepartments(prev => prev.map(d => {
      ...
      return { ...d, assignees: d.assignees.map(a => a.id === memberId ? { ...a, [field]: value } : a) };
    }));
  ...
  const handleSaveNote = () => {
    ...
    setDepartments(prev => prev.map(d => {
      ...
      updatedNotes[noteModal.index] = { id: noteModal.noteId || Date.now(), text: noteModal.text.trim(), assigneeId: noteModal.assigneeId };
  ...
  const handleDrop = (e, targetDeptId, targetIndex) => {
    ...
    try {
      ...
      setDepartments(prev => {
        ...
        return prev.map(d => {
          const updatedNotes = [...d.notes];
          if (d.id === sourceDeptId) updatedNotes[sourceIndex] = (sourceDeptId === targetDeptId) ? sourceDept.notes[targetIndex] : targetDept.notes[targetIndex];
          if (d.id === targetDeptId) updatedNotes[targetIndex] = movingNote;
          return { ...d, notes: updatedNotes };
        });
      });
  ...
  return (
    <div className="w-full text-left p-6 overflow-x-auto select-none flex flex-col gap-6">
          ...
          <div key={dept.id} className="flex-1 flex flex-col gap-3 min-w-[220px]">
            <div className="w-full group">
              <div className="flex justify-between items-start">
                <button onClick={() => setActiveMiniModule(dept)} className="text-left cursor-pointer focus:outline-hidden flex-1">
                  <div className="mb-2.5 mt-1 transition-transform group-hover:scale-105 duration-200">{getDepartmentIcon(dept.name)}</div>
                  <h3 className="text-lg font-bold tracking-tight text-[var(--text-h)] hover:text-[var(--color-primary)] transition-colors">{dept.name}</h3>
                </button>
              ...
              <p className="text-[10px] text-[var(--text)] leading-tight h-7 mt-1 overflow-hidden line-clamp-2 opacity-80">{dept.assets}</p>
            </div>
              ...
              <div className="space-y-2 mb-3 h-32 overflow-y-auto pr-1">
                {dept.todos.map((todo) => (
                  <label key={todo.id} className="flex items-start gap-2 text-xs text-[var(--color-foreground)] cursor-pointer select-none">
                    <input type="checkbox" checked={todo.completed} onChange={() => toggleTodo(dept.id, todo.id)} className="mt-0.5 rounded border-[var(--color-border)] text-[var(--color-primary)] text-[10px]" />
                    <span className={todo.completed ? 'line-through text-[var(--text)] opacity-50' : ''}>{todo.text}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                {dept.assignees.map((user) => (
                  ...
                  >
                    ...
                    {user.userId && <span className="text-green-400 text-[8px]">●</span>}
                  </button>
                ))}
                <button
      ...
      {noteModal.isOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-5 w-full max-w-sm shadow-xl flex flex-col gap-4 animate-fade-in">
            <h4 className="text-sm font-bold text-[var(--text-h)]">{noteModal.noteId ? '✏️ Modify Sticky Note' : '➕ Add Sticky Card'}</h4>
            <textarea autoFocus rows={3} value={noteModal.text} onChange={(e) => setNoteModal(prev => ({ ...prev, text: e.target.value }))} placeholder="Enter note context elements..." className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-input-background)] border border-[var(--color-border)] rounded p-2.5 focus:outline-hidden focus:border-[var(--color-ring)] resize-none" />
            <div>
              <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Assign to Member</label>
              <select value={noteModal.assigneeId} onChange={(e) => setNoteModal(prev => ({ ...prev, assigneeId: e.target.value }))} className="w-full text-xs bg-[var(--color-input-background)] text-[var(--color-foreground)] border border-[var(--color-border)] rounded p-1.5 focus:outline-hidden">
            ...
            <div className="flex justify-between items-center mt-2">
              {noteModal.noteId ? (
                <button type="button" onClick={handleDeleteNote} className="text-[11px] font-medium text-red-500 hover:underline cursor-pointer">Delete Note</button>
              ) : <div />}
              <div className="flex gap-2">
                <button type="button" onClick={() => setNoteModal({ isOpen: false, text: '', deptId: null, index: null, noteId: null, assigneeId: '' })} className="bg-[var(--color-muted)] text-[var(--color-foreground)] text-[11px] px-3 py-1.5 rounded hover:bg-[var(--color-secondary)] cursor-pointer">Cancel</button>
                <button type="button" onClick={handleSaveNote} className="bg-[var(--color-primary)] text-[var(--color-primary-foreground)] text-[11px] px-4 py-1.5 rounded cursor-pointer hover:opacity-90">Save</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {deptModal.isOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-5 w-full max-w-sm shadow-xl flex flex-col gap-4 animate-fade-in">
            <h4 className="text-sm font-bold text-[var(--text-h)]">{deptModal.mode === 'add' ? '➕ Add Department Track' : '✏️ Edit Department Details'}</h4>
            <div>
              <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Department Name</label>
              <input type="text" autoFocus placeholder="e.g. Frontend Development" value={deptModal.name} onChange={(e) => setDeptModal(prev => ({ ...prev, name: e.target.value }))} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-input-background)] border border-[var(--color-border)] rounded p-2 focus:outline-hidden focus:border-[var(--color-ring)]" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Assets Description Summary</label>
              <textarea rows={2} placeholder="e.g. Design files, mockups, documentation specs..." value={deptModal.assets} onChange={(e) => setDeptModal(prev => ({ ...prev, assets: e.target.value }))} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-input-background)] border border-[var(--color-border)] rounded p-2 focus:outline-hidden focus:border-[var(--color-ring)] resize-none" />
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-5 w-full max-w-sm shadow-xl flex flex-col gap-4 animate-fade-in">
            ...
            <div>
              ...
              <input type="text" autoFocus placeholder="e.g. John Doe" value={assigneeModal.name} onChange={(e) => setAssigneeModal(prev => ({ ...prev, name: e.target.value }))} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-input-background)] border border-[var(--color-border)] rounded p-2 focus:outline-hidden focus:border-[var(--color-ring)]" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Role Designation Title</label>
              <input type="text" placeholder="e.g. Lead Designer" value={assigneeModal.role} onChange={(e) => setAssigneeModal(prev => ({ ...prev, role: e.target.value }))} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-input-background)] border border-[var(--color-border)] rounded p-2 focus:outline-hidden focus:border-[var(--color-ring)]" />
            </div>
            <div className="flex justify-end gap-2 mt-2">
      ...
      {/* ── Member Allocation Side Drawer (with Calendar Availability) ── */}
      {activeMemberModule && (
          ...
          <div className="w-full max-w-sm bg-[var(--color-background)] h-full shadow-2xl p-6 overflow-y-auto flex flex-col border-l border-[var(--color-border)] animate-slide-in">
            ...
            <div className="flex flex-col gap-4 flex-1">
              <div>
                ...
                <input type="text" value={activeMemberModule.name} onChange={(e) => handleUpdateMemberField(activeMemberModule.deptId, activeMemberModule.id, 'name', e.target.value)} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-card)] border border-[var(--color-border)] p-2 rounded focus:outline-hidden" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Functional Role Designation</label>
                <input type="text" value={activeMemberModule.role} onChange={(e) => handleUpdateMemberField(activeMemberModule.deptId, activeMemberModule.id, 'role', e.target.value)} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-card)] border border-[var(--color-border)] p-2 rounded focus:outline-hidden" />
              </div>
              <div>
                ...
                <input type="text" placeholder="https://github.com/profile" value={activeMemberModule.link || ''} onChange={(e) => handleUpdateMemberField(activeMemberModule.deptId, activeMemberModule.id, 'link', e.target.value)} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-card)] border border-[var(--color-border)] p-2 rounded focus:outline-hidden" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider mb-1">Task Deliverable Log / Context</label>
                <textarea rows={3} placeholder="Describe metrics..." value={activeMemberModule.notes || ''} onChange={(e) => handleUpdateMemberField(activeMemberModule.deptId, activeMemberModule.id, 'notes', e.target.value)} className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-card)] border border-[var(--color-border)] p-2 rounded focus:outline-hidden resize-none" />
              </div>

              {/* ── Google Calendar Availability Section ── */}
              <div className="border-t border-[var(--color-border)] pt-4">
                <div className="flex items-center justify-between mb-3">
                  <label className="block text-[10px] font-bold text-[var(--text)] uppercase tracking-wider">
                    📅 Calendar Availability
                  </label>
                </div>

                <div className="mb-3">
                  <label className="block text-[10px] text-[var(--text)] mb-1">
                    Link to Reslv User ID <span className="opacity-50">(paste from MongoDB to load calendar)</span>
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. 6639f3a2b..."
                    value={activeMemberModule.userId || ''}
                    onChange={(e) => handleUpdateMemberField(activeMemberModule.deptId, activeMemberModule.id, 'userId', e.target.value)}
                    className="w-full text-xs text-[var(--color-foreground)] bg-[var(--color-card)] border border-[var(--color-border)] p-2 rounded focus:outline-hidden font-mono"
                  />
                </div>

                {activeMemberModule.userId?.trim() ? (
                  <CalendarAvailability userId={activeMemberModule.userId.trim()} />
                ) : (
                  <p className="text-[10px] text-[var(--text)] opacity-50 italic">
                    Enter a User ID above to load this member's Google Calendar availability for the sprint.
                  </p>
                )}
              </div>
              {/* ─────────────────────────────────────────── */}
